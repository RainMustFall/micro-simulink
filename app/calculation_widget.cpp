...
CalculationWidget::CalculationWidget(GraphController *controller,
  ...
  m_label = new QLabel("Press the button to get the graph result:");
  m_button = new QPushButton("Get Result");
...
void CalculationWidget::startCalculation() {
  if (!m_controller) {
    m_resultLineEdit->setText("Error: GraphController is not set.");
    return;
  }
  ...
  m_progressDialog =
      new QProgressDialog("Calculating...", "Cancel", 0, 0, this);
